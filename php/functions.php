...
function saml_acs() {
	...
	if (is_a($user_id, 'WP_Error')) {
		...
		// Check if Two Factor plugin is active and user has 2FA enabled
		$user = get_user_by('id', $user_id);
		if (class_exists('Two_Factor_Core') && Two_Factor_Core::is_user_using_two_factor($user_id)) {
			// Store SAML data in transient for use after 2FA verification
			$saml_data = array(
				'user_id' => $user_id,
				...
				'attrs' => $attrs,
				'newuser' => $newuser,
				'relay_state' => isset($_REQUEST['RelayState']) ? esc_url_raw($_REQUEST['RelayState'], ['https','http']) : null
			);
